import { useFilterContext } from '../../utils/MovieFilterContext'

const MovieSearchBox = () => {
  const { searchFilter, setSearchFilter } = useFilterContext()

  return (
    <label className='input input-sm w-full flex items-center gap-2 hover:ring-2 hover:ring-offset-1 hover:ring-secondary'>
      <input
        type='text'
        className='grow'
        placeholder='Search for movies..'
        value={searchFilter}
        onChange={e => setSearchFilter(e.target.value)}
      />
      {searchFilter && (
        <button
          className='btn btn-xs btn-ghost'
          aria-label='Clear Search Box'
          onClick={() => setSearchFilter('')}
        >
          <span className='hidden md:inline-flex'>Clear</span>
          <span className='md:hidden inline-flex'>&#xd7;</span>
        </button>
      )}
      <svg
        xmlns='http://www.w3.org/2000/svg'
        viewBox='0 0 16 16'
        fill='currentColor'
        className='h-4 w-4 opacity-70'
      >
        <path
          fillRule='evenodd'
          d='M9.965 11.026a5 5 0 1 1 1.06-1.06l2.755 2.754a.75.75 0 1 1-1.06 1.06l-2.755-2.754ZM10.5 7a3.5 3.5 0 1 1-7 0 3.5 3.5 0 0 1 7 0Z'
          clipRule='evenodd'
        />
      </svg>
    </label>
  )
}

export default MovieSearchBox
